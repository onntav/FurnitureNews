...
{	
	...
	public function getTotalPageVisitsPerIP($ipAddress)
	{
	...
	public function getMostVisitPagesPerIP($numberOfMostVisitedPagesToShow, $ipAddress)
    {
        $stmt = $this->connection->prepare(
			'SELECT PageNumber, 
				COUNT(*) AS VisitCount 
	...
	public function getCatalogVisitsSummary($pageNum, $pageSize, $sortCriteria, $searchVal, $groupByIpAddress = 0)
    {
		$sortOrder = strlen($sortCriteria) > 0 
				? $sortCriteria 
				: ' pv.VisitDateTime DESC,
			        PageNumber ASC, 
			        VisitCount DESC ';
					
		$groupBy = $groupByIpAddress == 1
			? ' cv.Id,  
				VisitDateTime'
			: ' cv.IpAddress, 
				pv.PageNumber, 
		...
		$query = 'SELECT 
			 ...
			 ' ORDER BY ' . $sortOrder . 
			 ' LIMIT ' . 
			 ($pageNum > 0 ? $pageNum : 0) . ',' . ($pageSize > 0 ? $pageSize : 0);
	...
	// $pageNum, $pageSize, $searchCol, $searchVal
	public function getAllCatalogVisitsSummary($pageNum, $pageSize, $sortCriteria, $searchVal, $groupByIpAddress = 0)
    {
		$sortOrder = strlen($sortCriteria) > 0 
				? $sortCriteria 
				: ' pv.VisitDateTime DESC,
