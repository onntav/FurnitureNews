...
{	
	private $db_config = array(
		"DatabaseName"=>	"tentecatalogue_new", 
		"Host"=>			"localhost",
		"UserName"=>		"rootlive",
		"Password"=>		"root1234"
	);
	...
	public function deletePerVisit($visitId)
	{
        $stmt = $this->connection->prepare('DELETE FROM pagevisit WHERE CatVisitId = ' . $visitId);
		$stmt->execute();
		$stmt = $this->connection->prepare('DELETE FROM catalogvisit WHERE Id = ' . $visitId);
		return $stmt->execute();
	}
