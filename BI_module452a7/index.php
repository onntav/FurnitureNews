...
<body>
	...
	<div class="title">
		<input onClick="deleteAllVisits()" type="button" class="green-button" value="Delete All Visits">
	</div>
		...
		<div id="tabs-1">
			...
			<script>
								...
								{	
									table = $('#tblPerUserBI').DataTable( {
										...
										"columnDefs": [ {
											"targets": -1,
											"data": null,
											"defaultContent": '<button class="btn_details" type="button">Details</button>'
												 + '&nbsp;&nbsp;<button class="btn_delete"  type="button">Del</button>'
										}]
									} );
							
							
									$('#tblPerUserBI tbody').on( 'click', '.btn_delete', function (d) {

										<?php
											if($groupPageVisitsByIpAddress == 0)
											{
												echo 'var visitId = $(this).parents(\'tr\').find("td:eq(4)").text();';
											}
											else
											{
												echo 'var visitId = $(this).parents(\'tr\').find("td:eq(3)").text();';
											}
										?>
									...
									$('#tblPerUserBI tbody').on( 'click', '.btn_details', function (d) {
										//alert(d.VisitId);
										...
										<?php
											if($groupPageVisitsByIpAddress == 0)
											{
												echo 'var customerId = $(this).parents(\'tr\').find("td:eq(5)").text();';
												echo 'var visitId = $(this).parents(\'tr\').find("td:eq(4)").text();';
											}
											else
											{
												echo 'var customerId = $(this).parents(\'tr\').find("td:eq(4)").text();';
												echo 'var visitId = $(this).parents(\'tr\').find("td:eq(3)").text();';
											}
										?>
										
										//var visitId = $(this).parents('tr').find("td:eq(3)").text();
										//var customerId = $(this).parents('tr').find("td:eq(4)").text();
										
										$.ajax({
											type: 'POST',
											url: 'speedometerData.php',
											data: ({IpAddress: ipAddress, VisitId: visitId, CustomerId: customerId}),
				...
				function deleteAllVisits(){
					var r = confirm("Are you sure you want to delete all catalogue visit records?");
					if (r == true) {
						$.ajax({
							type: 'POST',
							url: 'cleanUpVisits.php',
							success: function(result) {
								if(result > 0){
									location.reload();
								}
		...
		if($displayPerUserBI == 1){ 
			echo '<div id="tabs-2">
				<table id="tblPerUserBI" class="display" cellspacing="0" width="100%">
					<thead>
						<tr>
							...
							echo "<th style=\"width:0px;\">Visit Id</th>";
				...
				<div id="speedometers" class="gauge-container col-md-12 col-sx-12">
					...
					<h3 id="gg4Label" class="center">Number of pages visited</h3>
					</div>
					<div style="margin-top: 50px; width:100%; margin-left: auto; margin-right: auto;">
