...
<body>
	...
	<?php
		...
		{
			$biRepo = new BIRepository(null, $dbname);
			$actualNumOfVisits = $biRepo->getTotalCatalogVisit();
				
			echo
			'<script>'.
				'var numberOfVisits = ' . $numberOfVisits . ';' .
				'var actualNumOfVisits = ' . $actualNumOfVisits . ';' .
			...
			'<td width="30%"><div style="text-align:center;">'.
				'<div id="speedometerContainer" class="gauge-container col-md-12 col-sx-12">'.
					'<div id="gg'. $guageCtr . '" class="gauge col-md-6 col-sx-6"></div>'.
				'</div>'.
			'</div></td>';
			echo '</tr>';
		}
